...
@export var tiempo_antes_invocar : float = 0.6
...
func entrar():
	...
	if animacion.has_animation(enemigo.anim_demonio_invocar):
		animacion.stop()
		animacion.play(enemigo.anim_demonio_invocar)
	else:
		print("No existe animación de invocar, usando animación de ataque.")
...
func invocar_enemigos():
	if enemigo.escena_enemigo_invocado == null:
		print("ERROR: No hay escena_enemigo_invocado asignada")
		return
	...
	for i in range(cantidad):
		...
		print("Invocado enemigo en posicion: ", nuevo_enemigo.global_position)
